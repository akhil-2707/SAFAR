    ...
    >
      ...
      <div className="w-full max-w-[1680px] mx-auto px-3 sm:px-5 lg:px-6 xl:px-8 h-16 flex items-center justify-between gap-2 lg:gap-3.5">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          {/* Device Lifeline & Safety Permissions Quick Status */}
          {onOpenPermissionsModal && (
            <motion.button
              ...
              className={`px-2 sm:px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer border ${
                permissionStatus === 'granted'
                  ? 'bg-emerald-50 text-emerald-800 border-emerald-300 hover:bg-emerald-100'
                  : 'bg-amber-50 text-amber-800 border-amber-300 hover:bg-amber-100 animate-pulse'
              }`}
              title="Device Lifeline Safety Permissions (Location, Camera, Storage, BLE)"
            >
              <ShieldCheck className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
              <span className="hidden md:inline text-[11px]">
                {permissionStatus === 'granted' ? 'Lifeline Active' : 'Permissions'}
              </span>
            </motion.button>
          )}
                ...
                <div className="pt-2 border-t border-slate-200/60 mt-2 space-y-1">
                  {onOpenPermissionsModal && (
                    <button
                      ...
                      className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-semibold text-slate-700 hover:bg-slate-100 text-left transition-colors cursor-pointer"
                    >
                      <div className="flex items-center gap-3">
                        <ShieldCheck className="w-4 h-4 text-emerald-600" />
                        <span>Device Lifeline Permissions</span>
                      </div>
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                        ...
                        {permissionStatus === 'granted' ? '✓ Protected' : '⚠️ Pending'}
                      </span>
                    </button>
                  )}
                  {onOpenMeshModal && (
